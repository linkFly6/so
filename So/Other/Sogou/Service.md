...
- 服务模块（异步加载,二次加载）
 - Service.qo - 获取qo对象那个
 - Service.CallBacks和Service.DeferJsonp - 异步队列 
...
```javascript
	...
	//字符串补位
	console.log(service.padNumber('2', 3));//002
    console.log(service.padNumber('2', 3, 'a'));//aa2
	
	//判断原字符串开头是否是指定的字符串结尾，模拟String.prototype.startsWith - ECMAScript 2015(ES6) 规范
	console.log(service.startsWith('linkFly', 'link'));//true
    console.log(service.startsWith('linkFly', 'link', 2));//false

	
	//判断原字符串是否是指定的字符串结尾，模拟String.prototype.endsWith - ECMAScript 2015(ES6) 规范
    console.log(service.endsWith('linkFly', 'Fly'));//true
    console.log(service.endsWith('linkFly', 'Fly', 4));//false
...
重载
- service.truncate(target, length)
- service.truncate(target, length, truncation)
- service.truncate(target, length, isByte)
- service.truncate(target, length, truncation, isByte)
