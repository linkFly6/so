(function (window, undefined) {
    ...
    */
    ...
    //工具函数
    var isIndexOf = Array.prototype.indexOf,
    ...
    var callbacks = function (option) {
        ...
        var list = [],      //回调列表
            ...
            fired,          //是否执行过
            firingStart,    //当前已经执行过的回调列表索引，用于callbacks自动执行功能
            firingLength,   //回调函数的数组长度，用于维护firingStart的位置
            ...
            fire = function (data) {
                ...
                if (stack) //如果存在栈，则执行
                {
                    if (stack.length)//从栈头部取出，并递归fire()方法
                        fire(stack.shift());
                }
                if (auto)      //自动执行模式，则只清空list，而不清空auto里记录的最后的参数
                    list = [];
                else           //否则直接清空
                    self.disable();
            };
                ...
                if (list) {
                    ...
                    (function addCallback(args) {
                        each(list, function (item) {
                            if (isFunction(item)) {
                                list.push(item);
                            } else if (typeof item === '[object array]') {//这个判定抛弃了array-like
                                addCallback(item);
                            }
                        });
                    })(arguments);
